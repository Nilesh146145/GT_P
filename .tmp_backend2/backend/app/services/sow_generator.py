...
def run_hallucination_checks(wizard_data: Dict, steps_completed: List[int]) -> List[Dict]:
    ...
    layers.append({
        ...
        "active": l8_active,
        "status": "green" if (l8_active and signoff) else ("red" if l8_active else "grey"),
        "detail": f"UAT Sign-off Authority: {signoff}" if signoff else "UAT Sign-off Authority not designated (WIZ-009)."
    })
