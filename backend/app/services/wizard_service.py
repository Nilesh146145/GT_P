...
from app.core.database import get_wizards_collection, get_sows_collection
...
async def sow_action(sow_id: str, user_id: str, action: str, notes: Optional[str]) -> Dict:
    ...
    if action == "submit":
        ...
        # Delete this SOW, revert wizard
        await col.delete_one({"_id": ObjectId(sow_id)})
        wiz_col = get_wizards_collection()
        await wiz_col.update_one(
            {"wizard_id": sow["wizard_id"]},
            {"$set": {"status": WizardStatus.completed, "sow_id": None, "updated_at": datetime.utcnow()}}
        )
