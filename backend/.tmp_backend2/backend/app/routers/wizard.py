...
@router.put("/{wizard_id}/steps/0", response_model=StepSaveResponse,
            summary="Project Context & Discovery")
...
@router.put("/{wizard_id}/steps/1", response_model=StepSaveResponse,
            summary="Project Identity & Scope")
...
@router.put("/{wizard_id}/steps/2", response_model=StepSaveResponse,
            summary="Delivery & Technical Scope")
...
@router.put("/{wizard_id}/steps/3", response_model=StepSaveResponse,
            summary="Integrations & User Management")
...
async def get_step_9_summary(
    ...
    flow_reminders: List[str] = []
    if len(errors) == 0:
        flow_reminders.append(
            "On Generate, send business_owner_approver_id and final_approver_id in the JSON body."
        )
        flow_reminders.append(
            "After generation, use SOW Draft Review; Submit for approval is blocked while any hallucination layer is red."
        )
